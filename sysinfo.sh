...
#+----------------------------------------------------------
#| 20200319-074411 - MCT Michael Tiernan <Michael.Tiernan@gMail.com>
#|   Some mods for my own benefit and use.
#+----------------------------------------------------------
...
# Error out if any variables are unset. (Safety net for my own sanity.)
set -u
...
function _get_basic_info
{
	...
	currdate=$(date)
	_dateStamp=$(date +"%Y%m%dT%H%M%S")
	kernel=$(uname -sr)
}

function _if_ifconfig
{
...
function _set_docker_vars
{
	if [ ! -z "${has_docker-}" ] ; then
		# We checked and got a result.
		...
		if [ $has_docker -eq 1 ]; then
			if [[ $OSTYPE =~ "linux" ]]; then
				...
				_if_ifconfig	# Will exit if it doesn't exist.
